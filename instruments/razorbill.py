...
class _Channel(ChildInstrument):
    ...
    def wait_for(self):
        while self.voltage_now != self.voltage_set:
             time.sleep(0.05)

class RP100(ScpiInstrument):
    """
    RP100 Power Supply.
    Module for interfacing with the RP100 power supply. 
    """
    _idnstring = "Razorbill,RP100,"
    
    def _setup(self):
        self.channels = {1: _Channel(self, 1), 2: _Channel(self, 2)}
    ...
    diag_new_serial = _scpi_property("DIAG:SERIAL", '"{:06d}"', can_get=False, doc="set new serial number (reboot to take effect)")
